...
namespace eExNetworkLibrary.TrafficSplitting
{
    ...
    public class IPAddressRule : TrafficSplitterRule
    {
        ...
        public override bool IsMatch(eExNetworkLibrary.Ethernet.EthernetFrame ethFrame, eExNetworkLibrary.IP.IPv4Frame ipv4Frame, eExNetworkLibrary.UDP.UDPFrame udpFrame, eExNetworkLibrary.TCP.TCPFrame tcpFrame)
        {
            if (ipv4Frame != null)
            {
                if (lLogic == Logic.Or)
