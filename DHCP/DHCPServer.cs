...
namespace eExNetworkLibrary.DHCP
{
    ...
    public class DHCPServer : DirectInterfaceIOHandler
    {
        ...
        private IP.IPv4AddressAnalysis ipv4Analysis;
        ...
        public void AddToPool(DHCPPoolItem dhItem)
        {
            foreach (IPInterface ipi in lInterfaces)
            {
                for (int iC1 = 0; iC1 < ipi.IpAddresses.Length && iC1 < ipi.Subnetmasks.Length; iC1++)
                {
                    if (ipv4Analysis.GetClasslessNetworkAddress(ipi.IpAddresses[iC1], ipi.Subnetmasks[iC1]).Equals(ipv4Analysis.GetClasslessNetworkAddress(dhItem.Address, ipi.Subnetmasks[iC1])))
                    {
        ...
        protected virtual void CreatePool(IPAddress ipaStart, IPAddress ipaEnd, IPInterface ipi, int iAddrCounter)
        {
            ...
            IPAddress[] ipRange = ipv4Analysis.GetIPRange(ipaStart, ipaEnd);
        ...
        public DHCPServer()
        {
            ipv4Analysis = new eExNetworkLibrary.IP.IPv4AddressAnalysis();
        ...
        protected virtual void HandleDHCPFrame(DHCPFrame dhcFrame, eExNetworkLibrary.Ethernet.EthernetFrame ethFrame, eExNetworkLibrary.UDP.UDPFrame udpFrame, eExNetworkLibrary.IP.IPv4Frame ipv4Frame, TrafficDescriptionFrame tdf)
        {
